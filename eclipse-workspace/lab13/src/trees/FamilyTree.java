...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0) {
			throw new TreeException("Invalid File");		
			}
		...
		if (root == null) {
			...
			//throw new TreeException("Parent Node not found in data file.");
			if(parentNode == null) {
		...
		for(String a: childrenArray) {
			TreeNode child = new TreeNode(a.trim());
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("Invalid first name."); //Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("Invalid second name."); //Throw TreeException with a useful message
